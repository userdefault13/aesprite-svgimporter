-- Single USDC Working Converter
-- Uses a different approach for color replacement that actually works
...
-- Apply direct color replacement using a more robust method
local function applyDirectColors(svgString)
    local processedSVG = svgString
    
    -- Use a more robust replacement method
    -- Replace each class attribute individually
    ...
    for _ in processedSVG:gmatch('fill="' .. usdcColors.primaryColor .. '"') do
        primaryCount = primaryCount + 1
    end
    
    local secondaryCount = 0
    for _ in processedSVG:gmatch('fill="' .. usdcColors.secondaryColor .. '"') do
        secondaryCount = secondaryCount + 1
    end
    
    print("  Color replacements: " .. primaryCount .. " primary, " .. secondaryCount .. " secondary")
    
    -- Wrap in proper SVG document structure
    return string.format([[<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">%s</svg>]], processedSVG)
end
...
local function processSVGPart(partType, partIndex, svgString)
    ...
    -- Apply direct color replacement
    local wrappedSVG = applyDirectColors(svgString)
...
local function convertUSDCWorking()
    print("Starting USDC working conversion (robust color replacement)...")
    print("USDC colors:")
    ...
    -- Process hands parts
    print("=== Processing Hands Parts ===")
    if templates.hands and #templates.hands > 0 then
        for i, svgString in ipairs(templates.hands) do
            local success, pixelsPlaced = processSVGPart("hands", i - 1, svgString)
            if success then
    ...
    print("")
    print("USDC working conversion completed!")
