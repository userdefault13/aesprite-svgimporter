...
# Batch Import Collateral JSON
# Usage: ./batch-import-collateral.sh <json_file> [output_dir]
# Example: ./batch-import-collateral.sh ../AavegotchiQuerey/cli/exports/Collaterals/collateral-amaave-1764616862816.json

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT_FILE="$SCRIPT_DIR/batch-import-collateral-cli.lua"

if [ $# -lt 1 ]; then
    echo "Batch Import Collateral JSON"
    echo "============================"
    echo ""
    echo "Usage: $0 <json_file> [output_dir]"
    echo ""
    echo "Arguments:"
    echo "  json_file   - Path to collateral JSON file (required)"
    echo "  output_dir  - Output directory for .aseprite files (optional)"
    echo ""
    echo "Example:"
    echo "  $0 ../AavegotchiQuerey/cli/exports/Collaterals/collateral-amaave-1764616862816.json"
    echo ""
    exit 1
fi
