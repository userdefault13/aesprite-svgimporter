-- Batch All Collaterals Converter
-- Processes all 16 collaterals and organizes output into folders by collateral name
...
local function processCollateral(collateral, templates)
    ...
    if templates.body and #templates.body > 0 then
        local success, pixelsPlaced = processSVGPart("body", 0, templates.body[1], collateral, outputDir)
        if success then
            successCount = successCount + 1
            totalPixels = totalPixels + pixelsPlaced
        end
    ...
    if templates.mouth_neutral and #templates.mouth_neutral > 0 then
        local success, pixelsPlaced = processSVGPart("mouth_neutral", 0, templates.mouth_neutral[1], collateral, outputDir)
        if success then
            successCount = successCount + 1
            totalPixels = totalPixels + pixelsPlaced
        end
    ...
    if templates.mouth_happy and #templates.mouth_happy > 0 then
        local success, pixelsPlaced = processSVGPart("mouth_happy", 0, templates.mouth_happy[1], collateral, outputDir)
        if success then
            successCount = successCount + 1
            totalPixels = totalPixels + pixelsPlaced
        end
...
local function convertAllCollaterals()
    print("Starting batch conversion of all collaterals...")
    print("")
