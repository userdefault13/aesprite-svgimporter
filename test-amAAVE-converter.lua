-- Test converter for amAAVE SVGs with fixed color replacement and rect support
...
-- Improved color replacement that handles nested groups and removes style blocks
local function applyColors(svgString)
    local processedSVG = svgString
    
    -- Remove style blocks completely (we're replacing classes with fills)
    -- Use non-greedy matching to handle multiple style blocks
    processedSVG = processedSVG:gsub('<style>.-</style>', '')
    processedSVG = processedSVG:gsub('<style><!%[CDATA%[.-%]%]></style>', '')
    
    -- Replace classes on ALL elements (groups, paths, rects) - handle both single and multiple classes
    -- Order matters: handle more specific patterns first
    
    -- Handle groups with classes (most common case)
    processedSVG = processedSVG:gsub('(<g[^>]*class="gotchi%-primary"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-primary[^"]*"', 'fill="' .. amAAVEColors.primaryColor .. '"')
    end)
    processedSVG = processedSVG:gsub('(<g[^>]*class="gotchi%-secondary"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-secondary[^"]*"', 'fill="' .. amAAVEColors.secondaryColor .. '"')
    end)
    processedSVG = processedSVG:gsub('(<g[^>]*class="gotchi%-eyeColor"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-eyeColor[^"]*"', 'fill="' .. amAAVEColors.primaryColor .. '"')
    end)
    processedSVG = processedSVG:gsub('(<g[^>]*class="gotchi%-primary%-mouth"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-primary%-mouth[^"]*"', 'fill="' .. amAAVEColors.primaryColor .. '"')
    end)
    processedSVG = processedSVG:gsub('(<g[^>]*class="gotchi%-cheek"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-cheek[^"]*"', 'fill="' .. amAAVEColors.cheekColor .. '"')
    end)
    
    -- Handle path elements with classes
    processedSVG = processedSVG:gsub('(<path[^>]*class="gotchi%-primary"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-primary[^"]*"', 'fill="' .. amAAVEColors.primaryColor .. '"')
    end)
    processedSVG = processedSVG:gsub('(<path[^>]*class="gotchi%-secondary"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-secondary[^"]*"', 'fill="' .. amAAVEColors.secondaryColor .. '"')
    end)
    processedSVG = processedSVG:gsub('(<path[^>]*class="gotchi%-cheek"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-cheek[^"]*"', 'fill="' .. amAAVEColors.cheekColor .. '"')
    end)
    
    -- Handle rect elements with classes (for eyes)
    processedSVG = processedSVG:gsub('(<rect[^>]*class="gotchi%-eyeColor"[^>]*>)', function(match)
        return match:gsub('class="gotchi%-eyeColor[^"]*"', 'fill="' .. amAAVEColors.primaryColor .. '"')
    end)
    
    -- Fallback: replace any remaining class attributes that weren't caught above
