...
# Usage: ./batch-import-collateral-base.sh <json_file> [output_dir]
# Example: ./batch-import-collateral-base.sh ../AavegotchiQuerey/cli/exports/Body/collateral-base-amaave-1764615569457.json

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT_FILE="$SCRIPT_DIR/batch-import-collateral-base-cli.lua"

if [ $# -lt 1 ]; then
    echo "Batch Import Collateral Base JSON"
    echo "=================================="
    echo ""
    echo "Usage: $0 <json_file> [output_dir]"
    echo ""
    echo "Arguments:"
    echo "  json_file   - Path to collateral base JSON file (required)"
    echo "  output_dir  - Output directory for .aseprite files (optional)"
    echo ""
    echo "Example:"
    echo "  $0 ../AavegotchiQuerey/cli/exports/Body/collateral-base-amaave-1764615569457.json"
    echo ""
    exit 1
fi
