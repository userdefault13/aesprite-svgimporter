-- Single USDC Hands Poses Converter
-- Extracts specific hand poses and renders them with proper USDC colors
...
local function processHandPose(pose, svgString)
    ...
    -- Extract only the specific pose group from the SVG
    local poseGroup = ""
    if pose == "down_open" then
        poseGroup = svgString:match('<g class="gotchi%-handsDownOpen">(.-)</g>')
    elseif pose == "down_closed" then
        poseGroup = svgString:match('<g class="gotchi%-handsDownClosed">(.-)</g>')
    elseif pose == "up" then
    ...
    local coloredPose = poseGroup
    coloredPose = coloredPose:gsub('class="gotchi%-primary"', 'fill="' .. usdcColors.primaryColor .. '"')
    coloredPose = coloredPose:gsub('class="gotchi%-secondary"', 'fill="' .. usdcColors.secondaryColor .. '"')
    coloredPose = coloredPose:gsub('class="gotchi%-cheek"', 'fill="' .. usdcColors.cheekColor .. '"')
...
local function convertUSDCHandsPoses()
    print("Starting USDC hands poses conversion (pose extraction method)...")
    print("USDC colors:")
